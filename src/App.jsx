...
const TRANSLATIONS = {
  EN: {
    ...
    most_killer: "TOP COMBO", badge_legend: "Badge Legend", no_data: "No player data found yet.",
  ...
  ID: {
    ...
    most_killer: "JUARA COMBO", badge_legend: "Legenda Lencana", no_data: "Belum ada data pemain.",
...
export default function App() {
  ...
  // MENGGANTI boolean pointRushEnabled menjadi mode string
  ...
  useEffect(() => {
    ...
    gameState, winCondition, targetRounds, targetScore, actionCardsEnabled, pointMode, isReversed, overlapLength, isRandomOverlap, maxWordLength, activeChallenge, autoRestartEnabled, playerQueue
  ]);
      ...
      if (pointModeRef.current !== "OFF") {
        if (gameModeRef.current === "STEP_UP") {
            ...
            if (languageRef.current === "MIX") {
                // Cek apakah kata ada di kamus Inggris / Indonesia
                ...
                // Jika kata murni bahasa Inggris (tidak ada di kamus Indo), pakai rumus Inggris
                if (isEnglishWord && !isIndoWord) {
                    useIndoScoring = false;
                } else {
                    // Jika kata bahasa Indonesia (atau ada di kedua bahasa), prioritaskan rumus Indonesia
                    useIndoScoring = true;
                }
            }
            ...
            let scores;
            if (useIndoScoring) {
                // Poin Scrabble adaptasi Bahasa Indonesia
                scores = {
                ...
                // Poin standar Scrabble versi Inggris (Alfred Mosher Butts)
                scores = {
  ...
  const handleChatEvent = (data) => {
    ...
    if (gameStateRef.current === "PLAYING") {
      ...
      if (currentPlayer?.uniqueId === uniqueId && !currentPlayer.isEliminated) {
        let cleanWord = normalizeWord(lowerComment);
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-950 text-slate-200 font-sans overflow-hidden flex flex-col items-center justify-center p-2 sm:p-4 relative">
      <div className="absolute top-2 left-2 sm:top-4 sm:left-4 z-10 pointer-events-none">
        <h1 className="text-xl sm:text-3xl font-black text-sky-400 drop-shadow-sm">KOCAKATA</h1>
                ...
                <div className="animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-2">
                  ...
                  <div className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="flex items-center gap-1 bg-slate-800/50 rounded p-0.5 border border-slate-700">
                      ...
                      <span className="w-7 text-center font-mono font-bold text-slate-200 text-xs">{isRandomOverlap ? "RND" : overlapLength}</span>
                      <button onClick={() => {
          ...
          <div className="bg-slate-900 w-full max-w-2xl max-h-[80vh] rounded-2xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
              <div className="mb-6 bg-slate-800/40 rounded-xl p-3 border border-slate-700/50">
                <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3 flex items-center gap-2"><Info className="w-3 h-3" /> {t("badge_legend")}</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">👶</span> Pemula</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">🥇</span> 1+ Menang</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">🔥</span> 3+ Menang</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">👑</span> 10+ Menang</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">⭐</span> 5+ Main</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">⚡</span> 5+ Combo</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">🌟</span> 20+ Combo</div>
                  <div className="flex items-center gap-2 text-[10px] text-slate-400"><span className="text-sm">💎</span> 50+ Combo</div>
                </div>
              </div>
                      ...
                      <div key={stat.id} className={`relative flex items-center p-3 rounded-xl border ${rankStyle} transition-all hover:bg-slate-800 group`}>
                        ...
                        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 ml-2">
                          <div className="flex flex-col justify-center">
                            ...
                            <div className="flex gap-1 mt-1 flex-wrap">{stat.badges && stat.badges.map((b, i) => <span key={i} className="text-sm bg-slate-800 border border-slate-700 rounded px-1">{b}</span>)}</div>
                          </div>
                          <div className="flex flex-col justify-center gap-1.5 text-xs">
                            <div className="flex justify-between text-slate-400 mb-1">
                              <span title={t("stats_wins")}>🏆 <span className="text-amber-400 font-bold">{stat.wins || 0}</span></span>
                              <span title={t("stats_games")}>🎮 <span className="text-sky-400 font-bold">{stat.games || 0}</span></span>
                              <span title={t("stats_kills")}>⚡ <span className="text-emerald-400 font-bold">{stat.kills || 0}</span></span>
                            </div>
      ...
      <div className="transform scale-[0.85] -translate-y-12 sm:translate-y-0 sm:scale-100 transition-transform duration-300">
        <div className="relative w-[360px] h-[360px] sm:w-[500px] sm:h-[500px] flex items-center justify-center">
                          ...
                          return (
                            <div key={currentShowcase.word} className="flex flex-col items-center text-center max-w-[85%] bg-slate-800/50 p-4 sm:p-6 rounded-3xl border border-slate-700/50 shadow-md animate-in fade-in zoom-in duration-700 w-full">
                              <div className="flex items-center gap-2 mb-3 bg-slate-900 pr-4 pl-1.5 py-1.5 rounded-full border border-slate-800 shadow-sm">
                                ...
                                <span className="text-[10px] sm:text-xs text-slate-300 font-bold tracking-wide">Kocakata dari <span className="text-sky-400">{currentShowcase.nickname}</span></span>
                              </div>
                              <h3 className={`${currentShowcase.word.length > 14 ? 'text-lg sm:text-2xl' : currentShowcase.word.length > 9 ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-4xl'} font-black text-sky-400 tracking-widest uppercase mb-2 sm:mb-4 break-words w-full px-2`}>{currentShowcase.word}</h3>
                ...
                <div className="flex flex-col items-center">
                  {pointMode !== "OFF" && (
                    <div className="mb-3 flex justify-center animate-in slide-in-from-top fade-in duration-500">
                      {winCondition === "TIME" ? (
                        <div className={`px-4 py-1 rounded-full font-mono text-xl font-bold border shadow-sm ${globalTimer !== null && globalTimer <= 10 ? "text-red-400 border-red-800 bg-red-950/50 animate-pulse" : "text-sky-300 border-sky-800 bg-sky-950/50"}`}>
                  ...
                  <div className="flex items-center gap-1.5 mb-2 h-7">
                    ...
                    {pointMode !== "OFF" && (
                      <div title={`Point Mode: ${pointMode}`} className="w-6 h-6 rounded-full bg-emerald-950/50 text-emerald-400 flex items-center justify-center border border-emerald-800">
                  ...
                  <h2 className={`${currentWord?.length > 14 ? "text-xl sm:text-3xl" : currentWord?.length > 9 ? "text-2xl sm:text-4xl" : "text-3xl sm:text-5xl"} font-black text-slate-100 flex justify-center transition-all duration-300 px-2`}>
                    {(() => {
                      if (gameMode === "RHYME") {
                        return (
                          <div className="flex flex-col items-center">
                            <span className="text-4xl text-sky-400">...{targetRhyme.toUpperCase()}</span>
                            {currentWord && <span className="text-xs text-slate-500 mt-1 font-normal opacity-70">Terakhir: {currentWord.toUpperCase()}</span>}
                          </div>
                        );
                      }
                      if (gameMode === "FILL_BLANK") {
                         ...
                         return parts.map((part, i) => 
                            part === "..." 
                            ? <span key={i} className="text-emerald-500/80 mx-1 tracking-widest">. . .</span> 
                            : <span key={i} className="text-slate-100">{part}</span>
                         );
                      }
                      const { pre, high, post } = getDisplayParts(currentWord);
                      return (<><span>{pre.toUpperCase()}</span><span className="text-sky-400">{high.toUpperCase()}</span><span>{post.toUpperCase()}</span></>);
                    })()}
                  </h2>

                  <div className="flex flex-col items-center w-full">
                    <div className="mt-2 flex flex-col items-center gap-2 w-full max-w-[260px] sm:max-w-[380px]">
                      <div className="w-full font-mono text-sky-300 bg-sky-950/40 px-2 py-1.5 rounded-xl border border-sky-800/50 flex justify-center items-center text-center">
                        {(() => {
                          ...
                          return (
                            <div className={`${dynamicTextSize} flex flex-col justify-center items-center gap-y-1.5 leading-snug w-full`}>
                              <span className="font-semibold text-sky-200 px-1 text-center">{rule.desc}</span>
                              {gameMode !== "RHYME" && gameMode !== "FILL_BLANK" && (
                                <div className="flex items-center justify-center bg-slate-900 px-2 py-0.5 rounded-lg border border-sky-900/50 shadow-sm">
                                  {rule.action && <span className="mr-1.5 text-slate-500">→</span>}
                                  {rule.action && <span className="mr-1.5 font-sans text-slate-300">{rule.action}</span>}
                                  <span className="font-bold text-sky-400 bg-sky-900/30 px-2 py-0.5 rounded-md tracking-widest">{rule.target}</span>
                                </div>
                              )}
                            </div>
                          );
          ...
          {players.map((player, index) => {
            ...
            // Jarak avatar diperlebar di sini (radius) agar tidak menutupi teks tengah
            const radius = (gameState === "WAITING" && waitingCountdown === null) ? (isMobile ? 135 : 170) : (isMobile ? 185 : 260);
            ...
            return (
              <div key={player.uniqueId} className="absolute transition-all duration-500 ease-out flex flex-col items-center justify-center w-24 h-28" style={{ transform: `rotate(${angleDeg}deg) translate(${radius}px) rotate(-${angleDeg}deg)`, zIndex: isTurn ? 100 : 20 }}>
                      ...
                      return (
                        <div key={effect.id} className="absolute top-0 left-1/2 -translate-x-1/2 animate-flurry-gift flex flex-col items-center justify-center pointer-events-none w-[150px]">
                          {effect.giftPictureUrl ? (
                            ...
                            <span className="text-4xl drop-shadow-sm animate-bounce">🎁</span>
                          )}
                ...
                <div className={`relative group ${player.isEliminated ? "opacity-50 grayscale" : "opacity-100"}`}>
                  ...
                  {player.isBot && (
                    <div className={`absolute -top-1 -right-3 sm:-right-4 text-[7px] sm:text-[8px] px-1.5 py-0.5 rounded-full text-white font-bold border border-slate-900 z-30 flex items-center shadow-sm transition-colors ${
                      player.botDifficulty === 1 ? "bg-emerald-600" :
                      player.botDifficulty === 3 ? "bg-rose-600 animate-pulse" :
                      "bg-sky-600"
                    }`}>
                  ...
                  {isTurn && <div className={`absolute -top-6 left-1/2 -translate-x-1/2 font-bold text-xs px-2 py-0.5 rounded-full shadow-sm z-50 whitespace-nowrap border ${timer <= 5 ? "bg-red-900/80 text-red-200 animate-pulse border-red-700" : "bg-amber-900/80 text-amber-200 border-amber-700"}`}>{`${timer}s`}</div>}
                ...
                <div className="mt-[-12px] flex flex-col items-center z-40 relative w-full">
                  ...
                  {player.stats?.badges && player.stats.badges.length > 0 && (
                    <div className="flex flex-wrap justify-center gap-0.5 mt-1 animate-in slide-in-from-top-1 duration-300">
                       <div className="flex items-center bg-slate-900/80 px-1.5 py-0.5 rounded-full border border-slate-700 shadow-sm">
          ...
          {lastPlay && (
             <div key={lastPlay.id} className="absolute top-[10%] sm:top-[15%] left-1/2 -translate-x-1/2 pointer-events-none z-[250] flex flex-col items-center animate-float-up w-full">
                ...
                <div className={`${lastPlay.word.length > 12 ? 'text-xl sm:text-2xl' : lastPlay.word.length > 8 ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-5xl'} font-black text-emerald-400 drop-shadow-sm uppercase tracking-widest leading-none px-4 text-center`}>
                   {lastPlay.word}
                </div>
                {pointMode !== "OFF" && (
      ...
      {gameState === "ENDED" && getWinners().length > 0 && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in duration-500 p-4">
      ...
      <div className="absolute top-24 right-4 z-[120] flex flex-col gap-2 pointer-events-none items-end">
        {activeEffects.filter(e => !players.some(p => p.uniqueId === e.uniqueId)).map(effect => {
           if (effect.type === 'like') {
             return (
               <div key={effect.id} className="bg-slate-900 text-slate-300 px-3 py-1.5 rounded-full border border-rose-900/50 flex items-center gap-2 shadow-sm animate-in slide-in-from-right fade-in fade-out duration-500">
           ...
           if (effect.type === 'gift') {
             return (
               <div key={effect.id} className="bg-slate-900 text-slate-300 p-2 rounded-lg border border-pink-900/50 flex items-center gap-3 shadow-md animate-in slide-in-from-right fade-in fade-out duration-500">
