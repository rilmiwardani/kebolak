...
export default function App() {
  ...
  const hasPossibleAnswer = (startWord, specificOverlap = null) => {
    ...
    for (const candidate of dictionary) {
      ...
      if (validateConnection(startWord, candidate)) { found = true; break; }
    }
    ...
    if (isValid) {
      ...
      if ((gameModeRef.current === "DYNAMIC") && gameStateRef.current !== "ENDED") {
        ...
        if (turnCount + 1 >= Math.max(1, activePlayersCount)) {
          ...
          activeChallengeRef.current = selected; // Sinkronisasi langsung untuk validasi bot/hasPossibleAnswer
      ...
      if (isRandomOverlapRef.current) {
        ...
        if (validOverlaps.length > 0) {
          const nextOverlap = validOverlaps[Math.floor(Math.random() * validOverlaps.length)];
          setOverlapLength(nextOverlap);
          overlapLengthRef.current = nextOverlap;
        } else {
          setOverlapLength(1);
          overlapLengthRef.current = 1;
        }
      }
  ...
  function startGame() {
    ...
    if (isRandomOverlapRef.current && randomStart) {
      const maxOverlap = Math.min(4, randomStart.length);
      const validOverlaps = [];
      for (let i = 1; i <= maxOverlap; i++) {
        if (hasPossibleAnswer(randomStart, i)) validOverlaps.push(i);
      }
      if (validOverlaps.length > 0) {
        const nextOverlap = validOverlaps[Math.floor(Math.random() * validOverlaps.length)];
        setOverlapLength(nextOverlap);
  ...
  function toggleLanguage(forceReload = false) {
    ...
    if (targetLang === "ID") {
      ...
      fetch("/kamus.json").then((res) => res.json()).then((data) => {
        ...
        Object.keys(data).forEach((k) => {
          ...
          else { const clean = normalizeWord(k); if (clean) { dictSet.add(clean); if (data[k].nama) sMap[clean] = { nama: data[k].nama.toLowerCase() }; } }
        });
